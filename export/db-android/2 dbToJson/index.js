...
fs.copyFileSync('./lib/decodeMessageTable/Releases/decodeMessageTable.exe', './input/decodeMessageTable.exe');
exec('decodeMessageTable.exe', { cwd: path.join(__dirname, './input/'), encoding: 'utf8' });
fs.removeSync('./input/decodeMessageTable.exe');

console.log('FILTER_USER', FILTER_USER);
fs.mkdirpSync('./dist/BLOB/');

const msg = require('./input/message.json').filter(v => v.talker == FILTER_USER);
const res = msg.map(v => {
    const { msgId } = v;
...
fs.writeFileSync('./dist/db-android_msg_res.json', JSON.stringify(res, null, 4));
